...
- Treat `founders-office.sqlite3`, `Generated/`, `openloops.json`, `personalization.json`, `OPEN_LOOPS_CONTEXT.md`, photos, Codex runs, support bundles, and QA captures as private runtime data. They must never be committed.
...
- Use `Scripts/openloops.py` for local move-store updates only before a workspace is migrated. When `founders-office.sqlite3` exists, generated JSON is read-only and changes must use the app or a repository-authorized client. Never mark a move Done without evidence.
