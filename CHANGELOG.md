...
- Priority drag now begins on the persistent Moves document stack and finishes from the stable window root, so the first drag receives immediate lift feedback and still persists when auto-scroll recycles its original row or SwiftUI replaces the scroll host.
